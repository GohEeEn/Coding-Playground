<?php
    ...
    // Get user id as the reference to the data entry to be edited
    // Using GET method here as to enable connection for editing with dynamic row
    $idUser = $_GET["idUser"];

    // To avoid affection of SQL Injection , we have to sanitize the given data entry identifier (idUser here)
...
<html>
    <head>
        ...
        <title>MySQL Connect : Update user information</title>
    ...
    <body>
        ...
        <div class="container">
            ...
            <?php
                ...
                // SQL Query : SELECT every columns FROM table 'user' WHERE the row(s) has idUser with value $idUser
                $str_query = "SELECT * FROM user WHERE idUser=$idUser";

                // Return the whole data/values of str_query given and executed by database 
                $record = mysqli_query($mysql, $str_query);

                if(!$record)
                    echo "Errors : Unable to select user info from database<br>" . mysqli_error($mysql) . "<br>";
                else{
                    // Able to grab record from MySQL
                    
                    // Return each row of the dataset returned with the execution of mysqli_query
                    // Return the original dataset stored on database
        ...
        <div class="container mt-3">
            ...
            <form name="myForm" class="form-horizontal needs-validation" role="form" method="POST" action="#">
                ...
                <?php
                            ...
                            // SQL query to update the newly inserted data entry into given table
                            $str_query = "UPDATE user SET firstName='$firstName', lastName='$lastName', emailAddress='$emailAddress', password='$password' WHERE idUser=$idUser";
                ...
                ?>
                    ...
                    <div class="col-sm-10">
                        <div class="alert alert-success">
                                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                <strong>Success!</strong> The data entry has been updated !
                        </div>
                    </div>
                <?php
                            }
